...
/** Always-visible Pulse instructions (Ade intro) */
export default function PulseEmptyState() {
  const [expanded, setExpanded] = useState(false);

  return (
    <div className="rounded-xl border border-gray-200 bg-[#FFFDF8] p-4 sm:rounded-2xl sm:p-5 md:p-6">
      <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-start sm:gap-5 md:gap-6">
        <div className="shrink-0">
          <Image
            ...
            width={120}
            height={120}
            className="h-20 w-20 rounded-full border-2 border-brand-100 object-cover shadow-sm sm:h-[96px] sm:w-[96px] md:h-[120px] md:w-[120px]"
            priority
          />
        </div>
        ...
        <div className="min-w-0 w-full flex-1 text-center sm:text-left">
          ...
          {expanded ? (
            <div className="max-h-[min(50vh,28rem)] space-y-3 overflow-y-auto overscroll-contain text-left text-sm leading-relaxed text-gray-700 sm:max-h-none sm:overflow-visible md:text-base">
            ...
            <p className="line-clamp-3 text-sm leading-relaxed text-gray-700 sm:line-clamp-4 md:text-base">
          ...
          <button
            ...
            className="mt-3 inline-flex min-h-11 items-center justify-center rounded-lg px-3 text-sm font-semibold text-brand-500 transition-colors hover:bg-brand-50 hover:text-brand-600"
          >
